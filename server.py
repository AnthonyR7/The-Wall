...
def dashboard():
    ...
    message_query = "SELECT first_name, message,messages.user_id, messages.created_at, messages.updated_at FROM messages JOIN users ON messages.user_id = users.id"
    messages = mysql.query_db(message_query)
    comments_query = "SELECT first_name,last_name, comment,user_id,comments.created_at, message_id  FROM comments JOIN users ON messages.user_id = users.id"
...
def posts():
    ...
    return redirect(url_for("dashboard"))
@app.route("/comments/<inserted_id>", methods = ["POST"])
def comments(inserted_id):
